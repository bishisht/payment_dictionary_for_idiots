...
\dictentry{Encryption}{/ɛnˈkrɪpʃən/, \textit{n.}}{
The process of converting information or data into a code, especially to
prevent unauthorized access. In payments, encryption is used to protect
sensitive data like card numbers and PINs as they travel across the network.
Only the party with the correct "key" can decode the information back into its
original form.

\example{When you buy something online, your browser uses encryption to "lock"
your credit card information so that only the merchant's bank can unlock and
read it.}

\analogy{Everyday}{Like a \textbf{Lock and Key}. Imagine you want to send a
secret message to a friend in a box. You put the message in the box and lock it
with a padlock. Anyone who sees the box can see it exists, but they can't read
the message inside without the specific key that fits the lock.}

\crossref{\textbf{DUKPT}, \textbf{HSM}, \textbf{Tokenization}}

\etymology{From Greek "kryptos" (hidden) + English prefix "en-" (to make). Used
in various forms for thousands of years, from ancient ciphers to modern digital
algorithms.}
}
\indexentry{Encryption}
